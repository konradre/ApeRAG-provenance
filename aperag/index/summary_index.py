...
class SummaryIndexer(BaseIndexer):
    ...
    def create_index(self, document_id: str, content: str, doc_parts: List[Any], collection, **kwargs) -> IndexResult:
        ...
        try:
            ...
            # Record SUMMARY_TEXT provenance
            run_id = kwargs.get("run_id")
            parse_provenance_id = kwargs.get("parse_provenance_id")
            summary_provenance_id = None
            if run_id:
                try:
                    ...
                    summary_hash = compute_content_hash(summary)
                    summary_node = prov.create_node(
                        document_id=document_id,
                        collection_id=collection.id,
                        run_id=run_id,
                        artifact_type=ArtifactType.SUMMARY_TEXT.value,
                        artifact_key=f"{document_id}:summary:{summary_hash[:8]}",
                        content_hash=summary_hash,
                        processor="summary_generator",
                        processor_version="1.0",
                        primary_parent_id=parse_provenance_id,
                    )
                    if summary_node:
                        summary_provenance_id = summary_node.id
                except Exception:
                    logger.debug("Failed to record summary provenance for doc %s", document_id)
            ...
            try:
                ...
                summary_ctx_ids = create_embeddings_and_store(
                    ...
                    collection_id=collection.id,
                    run_id=run_id,
                    parse_provenance_id=summary_provenance_id,
                )

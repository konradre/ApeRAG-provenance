...
async def get_document_provenance(
    ...
    Traces all derived artifacts (chunks, embeddings, entities, summaries, images)
    back to the original uploaded document through the complete derivation chain.

    Args:
        document_id: The ID of the document to get provenance for

    Returns:
        Provenance DAG with nodes, materializations, and DAG inputs
    """
    try:
        api_key = get_api_key()
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.get(
                f"{API_BASE_URL}/api/v1/documents/{document_id}/provenance",
                headers={"Authorization": f"Bearer {api_key}"},
            )
            if response.status_code == 200:
                return response.json()
            else:
                return {"error": f"Failed to fetch provenance: {response.status_code}", "details": response.text}
    except ValueError as e:
        return {"error": str(e)}
...
async def get_provenance_chain(
    ...
    Walks the primary parent chain to produce a linear path from the root
    DOCUMENT_VERSION node to the specified leaf node. Useful for understanding
    how a specific search result was derived.

    Args:
        node_id: The provenance node ID (starts with 'pn_')

    Returns:
        Ordered chain of provenance nodes from root to the specified node
    """
    try:
        api_key = get_api_key()
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.get(
                f"{API_BASE_URL}/api/v1/provenance/{node_id}/chain",
                headers={"Authorization": f"Bearer {api_key}"},
            )
            if response.status_code == 200:
                return response.json()
            else:
                return {"error": f"Failed to fetch provenance chain: {response.status_code}", "details": response.text}
    except ValueError as e:
        return {"error": str(e)}
